# CYBERWATCH SAS - 2016
#
# Security fix for CESA-2007:0731
#
# Security announcement date: 2007-08-02 00:30:43 UTC
# Script generation date:     2016-02-04 19:09:58 UTC
#
# Operating System: CentOS 5
# Architecture: x86_64
#
# Vulnerable packages fix on version:
#   - tetex.x86_64:3.0-33.1.el5
#   - tetex-afm.x86_64:3.0-33.1.el5
#   - tetex-doc.x86_64:3.0-33.1.el5
#   - tetex-dvips.x86_64:3.0-33.1.el5
#   - tetex-fonts.x86_64:3.0-33.1.el5
#   - tetex-latex.x86_64:3.0-33.1.el5
#   - tetex-xdvi.x86_64:3.0-33.1.el5
#   - tzdata.noarch:2007f-1.el2_1
#   - tetex.i386:1.0.7-38.5E.11
#   - tetex-afm.i386:1.0.7-38.5E.11
#   - tetex-doc.i386:1.0.7-38.5E.11
#   - tetex-dvilj.i386:1.0.7-38.5E.11
#   - tetex-dvips.i386:1.0.7-38.5E.11
#   - tetex-fonts.i386:1.0.7-38.5E.11
#   - tetex-latex.i386:1.0.7-38.5E.11
#   - tetex-xdvi.i386:1.0.7-38.5E.11
#   - kernel-BOOT.i386:2.4.9-e.72
#   - kernel-doc.i386:2.4.9-e.72
#   - kernel-headers.i386:2.4.9-e.72
#   - kernel-source.i386:2.4.9-e.72
#   - kernel.i686:2.4.9-e.72
#   - kernel-debug.i686:2.4.9-e.72
#   - kernel-enterprise.i686:2.4.9-e.72
#   - kernel-smp.i686:2.4.9-e.72
#   - kernel-summit.i686:2.4.9-e.72
#
# Last versions recommanded by security team:
#   - tetex.x86_64:3.0-33.15.el5_8.1
#   - tetex-afm.x86_64:3.0-33.15.el5_8.1
#   - tetex-doc.x86_64:3.0-33.15.el5_8.1
#   - tetex-dvips.x86_64:3.0-33.15.el5_8.1
#   - tetex-fonts.x86_64:3.0-33.15.el5_8.1
#   - tetex-latex.x86_64:3.0-33.15.el5_8.1
#   - tetex-xdvi.x86_64:3.0-33.15.el5_8.1
#   - tzdata.noarch:2008e-1.el2_1
#   - tetex.i386:1.0.7-38.5E.12
#   - tetex-afm.i386:1.0.7-38.5E.12
#   - tetex-doc.i386:1.0.7-38.5E.12
#   - tetex-dvilj.i386:1.0.7-38.5E.12
#   - tetex-dvips.i386:1.0.7-38.5E.12
#   - tetex-fonts.i386:1.0.7-38.5E.12
#   - tetex-latex.i386:1.0.7-38.5E.12
#   - tetex-xdvi.i386:1.0.7-38.5E.12
#   - kernel-BOOT.i386:2.4.9-e.72
#   - kernel-doc.i386:2.4.9-e.72
#   - kernel-headers.i386:2.4.9-e.72
#   - kernel-source.i386:2.4.9-e.72
#   - kernel.i686:2.4.9-e.72
#   - kernel-debug.i686:2.4.9-e.72
#   - kernel-enterprise.i686:2.4.9-e.72
#   - kernel-smp.i686:2.4.9-e.72
#   - kernel-summit.i686:2.4.9-e.72
#
# CVE List:
#
# More details:
#   - https://www.cyberwatch.fr/notices/CESA-2007:0731
#
# Licence: Released under The MIT License (MIT), See LICENSE FILE
sudo yum install tetex.x86_64-3.0 -y 
sudo yum install tetex-afm.x86_64-3.0 -y 
sudo yum install tetex-doc.x86_64-3.0 -y 
sudo yum install tetex-dvips.x86_64-3.0 -y 
sudo yum install tetex-fonts.x86_64-3.0 -y 
sudo yum install tetex-latex.x86_64-3.0 -y 
sudo yum install tetex-xdvi.x86_64-3.0 -y 
sudo yum install tzdata.noarch-2008e -y 
sudo yum install tetex.i386-1.0.7 -y 
sudo yum install tetex-afm.i386-1.0.7 -y 
sudo yum install tetex-doc.i386-1.0.7 -y 
sudo yum install tetex-dvilj.i386-1.0.7 -y 
sudo yum install tetex-dvips.i386-1.0.7 -y 
sudo yum install tetex-fonts.i386-1.0.7 -y 
sudo yum install tetex-latex.i386-1.0.7 -y 
sudo yum install tetex-xdvi.i386-1.0.7 -y 
sudo yum install kernel-BOOT.i386-2.4.9 -y 
sudo yum install kernel-doc.i386-2.4.9 -y 
sudo yum install kernel-headers.i386-2.4.9 -y 
sudo yum install kernel-source.i386-2.4.9 -y 
sudo yum install kernel.i686-2.4.9 -y 
sudo yum install kernel-debug.i686-2.4.9 -y 
sudo yum install kernel-enterprise.i686-2.4.9 -y 
sudo yum install kernel-smp.i686-2.4.9 -y 
sudo yum install kernel-summit.i686-2.4.9 -y 
