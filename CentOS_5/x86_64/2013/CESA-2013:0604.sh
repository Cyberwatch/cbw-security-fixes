# CYBERWATCH SAS - 2015
#
# Security fix for CESA-2013:0604
#
# Security announcement date: 2013-03-06 21:15:03 UTC
# Script generation date:     2015-11-19 07:05:20 UTC
#
# Operating System: CentOS 5
# Architecture: x86_64
#
# Vulnerable packages fix on version:
#   - java-1.6.0-openjdk:1.6.0.0-1.36.1.11.9.el5_9
#   - java-1.6.0-openjdk-demo:1.6.0.0-1.36.1.11.9.el5_9
#   - java-1.6.0-openjdk-devel:1.6.0.0-1.36.1.11.9.el5_9
#   - java-1.6.0-openjdk-javadoc:1.6.0.0-1.36.1.11.9.el5_9
#   - java-1.6.0-openjdk-src:1.6.0.0-1.36.1.11.9.el5_9
#
# Last versions recommanded by security team:
#   - java-1.6.0-openjdk:1.6.0.37-1.13.9.4.el5_11
#   - java-1.6.0-openjdk-demo:1.6.0.37-1.13.9.4.el5_11
#   - java-1.6.0-openjdk-devel:1.6.0.37-1.13.9.4.el5_11
#   - java-1.6.0-openjdk-javadoc:1.6.0.37-1.13.9.4.el5_11
#   - java-1.6.0-openjdk-src:1.6.0.37-1.13.9.4.el5_11
#
# CVE List:
#   - CVE-2013-0809
#   - CVE-2013-1493
#
# More details:
#   - https://www.cyberwatch.fr/notices/CESA-2013:0604
#
# Licence: Released under The MIT License (MIT), See LICENSE FILE
sudo yum install java-1.6.0-openjdk-1.6.0.37 -y 
sudo yum install java-1.6.0-openjdk-demo-1.6.0.37 -y 
sudo yum install java-1.6.0-openjdk-devel-1.6.0.37 -y 
sudo yum install java-1.6.0-openjdk-javadoc-1.6.0.37 -y 
sudo yum install java-1.6.0-openjdk-src-1.6.0.37 -y 
