# CYBERWATCH SAS - 2016
#
# Security fix for CESA-2016:0009
#
# Security announcement date: 2016-01-07 22:28:49 UTC
# Script generation date:     2016-02-04 19:13:15 UTC
#
# Operating System: CentOS 7
# Architecture: x86_64
#
# Vulnerable packages fix on version:
#   - ldb-tools.x86_64:1.1.20-1.el7_2.2
#   - libldb.x86_64:1.1.20-1.el7_2.2
#   - libldb-devel.x86_64:1.1.20-1.el7_2.2
#   - pyldb.x86_64:1.1.20-1.el7_2.2
#   - pyldb-devel.x86_64:1.1.20-1.el7_2.2
#   - libldb.i686:1.1.20-1.el7_2.2
#   - libldb-devel.i686:1.1.20-1.el7_2.2
#   - pyldb.i686:1.1.20-1.el7_2.2
#   - pyldb-devel.i686:1.1.20-1.el7_2.2
#
# Last versions recommanded by security team:
#   - ldb-tools.x86_64:1.1.20-1.el7_2.2
#   - libldb.x86_64:1.1.20-1.el7_2.2
#   - libldb-devel.x86_64:1.1.20-1.el7_2.2
#   - pyldb.x86_64:1.1.20-1.el7_2.2
#   - pyldb-devel.x86_64:1.1.20-1.el7_2.2
#   - libldb.i686:1.1.20-1.el7_2.2
#   - libldb-devel.i686:1.1.20-1.el7_2.2
#   - pyldb.i686:1.1.20-1.el7_2.2
#   - pyldb-devel.i686:1.1.20-1.el7_2.2
#
# CVE List:
#   - CVE-2015-3223
#   - CVE-2015-5330
#
# More details:
#   - https://www.cyberwatch.fr/notices/CESA-2016:0009
#
# Licence: Released under The MIT License (MIT), See LICENSE FILE
sudo yum install ldb-tools.x86_64-1.1.20 -y 
sudo yum install libldb.x86_64-1.1.20 -y 
sudo yum install libldb-devel.x86_64-1.1.20 -y 
sudo yum install pyldb.x86_64-1.1.20 -y 
sudo yum install pyldb-devel.x86_64-1.1.20 -y 
sudo yum install libldb.i686-1.1.20 -y 
sudo yum install libldb-devel.i686-1.1.20 -y 
sudo yum install pyldb.i686-1.1.20 -y 
sudo yum install pyldb-devel.i686-1.1.20 -y 
