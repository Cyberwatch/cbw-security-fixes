# CYBERWATCH SAS - 2016
#
# Security fix for RHSA-2015:0807
#
# Security announcement date: 2015-04-15 18:16:24 UTC
# Script generation date:     2016-01-06 19:13:40 UTC
#
# Operating System: Red Hat 5
# Architecture: x86_64
#
# Vulnerable packages fix on version:
#   - java-1.7.0-openjdk:1.7.0.79-2.5.5.2.el5_11.x86_64
#   - java-1.7.0-openjdk-debuginfo:1.7.0.79-2.5.5.2.el5_11.x86_64
#   - java-1.7.0-openjdk-demo:1.7.0.79-2.5.5.2.el5_11.x86_64
#   - java-1.7.0-openjdk-devel:1.7.0.79-2.5.5.2.el5_11.x86_64
#   - java-1.7.0-openjdk-javadoc:1.7.0.79-2.5.5.2.el5_11.x86_64
#   - java-1.7.0-openjdk-src:1.7.0.79-2.5.5.2.el5_11.x86_64
#
# Last versions recommanded by security team:
#   - java-1.7.0-openjdk:1.7.0.91-2.6.2.1.el5_11.x86_64
#   - java-1.7.0-openjdk-debuginfo:1.7.0.91-2.6.2.1.el5_11.x86_64
#   - java-1.7.0-openjdk-demo:1.7.0.91-2.6.2.1.el5_11.x86_64
#   - java-1.7.0-openjdk-devel:1.7.0.91-2.6.2.1.el5_11.x86_64
#   - java-1.7.0-openjdk-javadoc:1.7.0.91-2.6.2.1.el5_11.x86_64
#   - java-1.7.0-openjdk-src:1.7.0.91-2.6.2.1.el5_11.x86_64
#
# CVE List:
#   - CVE-2005-1080
#   - CVE-2015-0460
#   - CVE-2015-0469
#   - CVE-2015-0477
#   - CVE-2015-0478
#   - CVE-2015-0480
#   - CVE-2015-0488
#
# More details:
#   - https://www.cyberwatch.fr/notices/RHSA-2015:0807
#
# Licence: Released under The MIT License (MIT), See LICENSE FILE
sudo yum install java-1.7.0-openjdk-1.7.0.91 -y 
sudo yum install java-1.7.0-openjdk-debuginfo-1.7.0.91 -y 
sudo yum install java-1.7.0-openjdk-demo-1.7.0.91 -y 
sudo yum install java-1.7.0-openjdk-devel-1.7.0.91 -y 
sudo yum install java-1.7.0-openjdk-javadoc-1.7.0.91 -y 
sudo yum install java-1.7.0-openjdk-src-1.7.0.91 -y 
