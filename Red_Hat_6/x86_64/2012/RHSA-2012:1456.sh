#!/bin/sh
# CYBERWATCH SAS - 2017
#
# Security fix for RHSA-2012:1456
#
# Security announcement date: 2012-11-12 18:54:41 UTC
# Script generation date:     2017-01-01 21:14:16 UTC
#
# Operating System: Red Hat 6
# Architecture: x86_64
#
# Vulnerable packages fix on version:
#   - sos.noarch:2.2-17.1.el6rhs
#   - glusterfs.x86_64:3.3.0.5rhs-37.el6rhs
#   - glusterfs-debuginfo.x86_64:3.3.0.5rhs-37.el6rhs
#   - glusterfs-devel.x86_64:3.3.0.5rhs-37.el6rhs
#   - glusterfs-fuse.x86_64:3.3.0.5rhs-37.el6rhs
#   - glusterfs-geo-replication.x86_64:3.3.0.5rhs-37.el6rhs
#   - glusterfs-rdma.x86_64:3.3.0.5rhs-37.el6rhs
#   - glusterfs-server.x86_64:3.3.0.5rhs-37.el6rhs
#   - glusterfs.x86_64:3.3.0.5rhs-37.el6
#   - glusterfs-debuginfo.x86_64:3.3.0.5rhs-37.el6
#   - glusterfs-devel.x86_64:3.3.0.5rhs-37.el6
#   - glusterfs-fuse.x86_64:3.3.0.5rhs-37.el6
#   - glusterfs-rdma.x86_64:3.3.0.5rhs-37.el6
#
# Last versions recommanded by security team:
#   - sos.noarch:2.2-17.2.el6rhs
#   - glusterfs.x86_64:3.7.1-16.el6
#   - glusterfs-debuginfo.x86_64:3.7.1-16.el6
#   - glusterfs-devel.x86_64:3.7.1-16.el6
#   - glusterfs-fuse.x86_64:3.7.1-16.el6
#   - glusterfs-geo-replication.x86_64:3.7.1-16.el6rhs
#   - glusterfs-rdma.x86_64:3.7.1-16.el6
#   - glusterfs-server.x86_64:3.7.1-16.el6rhs
#   - glusterfs.x86_64:3.7.1-16.el6
#   - glusterfs-debuginfo.x86_64:3.7.1-16.el6
#   - glusterfs-devel.x86_64:3.7.1-16.el6
#   - glusterfs-fuse.x86_64:3.7.1-16.el6
#   - glusterfs-rdma.x86_64:3.7.1-16.el6
#
# CVE List:
#   - CVE-2012-4417
#
# More details:
#   - https://www.cyberwatch.fr/vulnerabilites
#
# Licence: Released under The MIT License (MIT), See LICENSE FILE
sudo yum install sos.noarch-2.2 -y 
sudo yum install glusterfs.x86_64-3.7.1 -y 
sudo yum install glusterfs-debuginfo.x86_64-3.7.1 -y 
sudo yum install glusterfs-devel.x86_64-3.7.1 -y 
sudo yum install glusterfs-fuse.x86_64-3.7.1 -y 
sudo yum install glusterfs-geo-replication.x86_64-3.7.1 -y 
sudo yum install glusterfs-rdma.x86_64-3.7.1 -y 
sudo yum install glusterfs-server.x86_64-3.7.1 -y 
sudo yum install glusterfs.x86_64-3.7.1 -y 
sudo yum install glusterfs-debuginfo.x86_64-3.7.1 -y 
sudo yum install glusterfs-devel.x86_64-3.7.1 -y 
sudo yum install glusterfs-fuse.x86_64-3.7.1 -y 
sudo yum install glusterfs-rdma.x86_64-3.7.1 -y 
