# CYBERWATCH SAS - 2016
#
# Security fix for RHSA-2014:0902
#
# Security announcement date: 2014-07-18 01:50:49 UTC
# Script generation date:     2016-02-04 19:17:53 UTC
#
# Operating System: Red Hat 7
# Architecture: x86_64
#
# Vulnerable packages fix on version:
#   - java-1.7.0-oracle.x86_64:1.7.0.65-1jpp.1.el7
#   - java-1.7.0-oracle-devel.x86_64:1.7.0.65-1jpp.1.el7
#   - java-1.7.0-oracle-javafx.x86_64:1.7.0.65-1jpp.1.el7
#   - java-1.7.0-oracle-jdbc.x86_64:1.7.0.65-1jpp.1.el7
#   - java-1.7.0-oracle-plugin.x86_64:1.7.0.65-1jpp.1.el7
#   - java-1.7.0-oracle-src.x86_64:1.7.0.65-1jpp.1.el7
#   - java-1.7.0-oracle.i686:1.7.0.65-1jpp.1.el7
#   - java-1.7.0-oracle-devel.i686:1.7.0.65-1jpp.1.el7
#
# Last versions recommanded by security team:
#   - java-1.7.0-oracle.x86_64:1.7.0.95-1jpp.2.el7
#   - java-1.7.0-oracle-devel.x86_64:1.7.0.95-1jpp.2.el7
#   - java-1.7.0-oracle-javafx.x86_64:1.7.0.95-1jpp.2.el7
#   - java-1.7.0-oracle-jdbc.x86_64:1.7.0.95-1jpp.2.el7
#   - java-1.7.0-oracle-plugin.x86_64:1.7.0.95-1jpp.2.el7
#   - java-1.7.0-oracle-src.x86_64:1.7.0.95-1jpp.2.el7
#   - java-1.7.0-oracle.i686:1.7.0.95-1jpp.2.el7
#   - java-1.7.0-oracle-devel.i686:1.7.0.95-1jpp.2.el7
#
# CVE List:
#   - CVE-2014-2483
#   - CVE-2014-2490
#   - CVE-2014-4208
#   - CVE-2014-4209
#   - CVE-2014-4216
#   - CVE-2014-4218
#   - CVE-2014-4219
#   - CVE-2014-4220
#   - CVE-2014-4221
#   - CVE-2014-4223
#   - CVE-2014-4227
#   - CVE-2014-4244
#   - CVE-2014-4252
#   - CVE-2014-4262
#   - CVE-2014-4263
#   - CVE-2014-4264
#   - CVE-2014-4265
#   - CVE-2014-4266
#
# More details:
#   - https://www.cyberwatch.fr/notices/RHSA-2014:0902
#
# Licence: Released under The MIT License (MIT), See LICENSE FILE
sudo yum install java-1.7.0-oracle.x86_64-1.7.0.95 -y 
sudo yum install java-1.7.0-oracle-devel.x86_64-1.7.0.95 -y 
sudo yum install java-1.7.0-oracle-javafx.x86_64-1.7.0.95 -y 
sudo yum install java-1.7.0-oracle-jdbc.x86_64-1.7.0.95 -y 
sudo yum install java-1.7.0-oracle-plugin.x86_64-1.7.0.95 -y 
sudo yum install java-1.7.0-oracle-src.x86_64-1.7.0.95 -y 
sudo yum install java-1.7.0-oracle.i686-1.7.0.95 -y 
sudo yum install java-1.7.0-oracle-devel.i686-1.7.0.95 -y 
